...
We use Flask to build an API that handles requests from our frontend, and returns responses back to the client. The data in the API request contains the document in string form, and an associated list of questions. The API response consists of a list of question-answer pairs. The backend also calls the model with the inputted data.
